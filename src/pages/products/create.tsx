...
export const ProductCreate = () => {
  ...
  const onSubmit = (data) => {
    ...
    for (let i = 0; i < photos.length; i++) {
      formData.append('photos', photos[i].file); // Добавляем файл в formData
    }
    ...
    const attributeData = validAttributes.map((attr) => ({
      nameKZ: attr.nameKZ,
      nameRU: attr.nameRU,
      nameEN: attr.nameEN,
      attributeId: attr.id,
    ...
    formData.append('attributeValues', JSON.stringify(attributeData));
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      ...
      setPhotos((prevPhotos) => [...prevPhotos, { file, url: imageURL }]); // Добавляем и файл, и его URL
    } else {
